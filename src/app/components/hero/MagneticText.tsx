...
export function MagneticText({
  ...
  useEffect(() => {
    ...
    if (!container) return;
    const updateSize = () => setContainerSize({ width: container.offsetWidth, height: container.offsetHeight });
    updateSize();
  ...
  return (
    ...
    >
      <span className={`relative z-0 ${textClassName}`}>{text}</span>
      <span
        ref={circleRef}
        aria-hidden
      ...
      >
        <span
          ...
          style={{ width: containerSize.width, height: containerSize.height, left: "50%", top: "50%", willChange: "transform" }}
        >
          <span className={`whitespace-nowrap text-white ${textClassName}`}>{hoverText}</span>
        </span>
      </span>
    </span>
  );
